...
/**
 * The one real email-sender job, shown as a small chip directly under whichever producer
 * queued the email — deliberately NOT styled like PipelineNode's full card, so it reads as
 * "a side-effect of the block above", not a 4th main-chain stage. See EMAIL_BADGES' doc
 * comment in pipelineGraph.ts for why there are 3 of these instead of one shared box.
 */
export function EmailBadgeNode({ data }: NodeProps<Node<EmailBadgeNodeData>>) {
  return (
    <div className="w-[220px] rounded-full border border-slate-900/5 bg-white/90 px-3 py-2 shadow-sm backdrop-blur-xl">
      <Handle type="target" id="target-top" position={Position.Top} className="!h-0 !w-0 !min-w-0 !border-0 !bg-transparent" />
