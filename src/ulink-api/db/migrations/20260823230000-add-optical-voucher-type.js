...
// Adds "optical" to invoices.items[].voucher_type's enum. Verified against real sample data:
// incomplete/jd2 (Khin Maung) is an eyewear voucher (optical shop frame/lens order slip with
// SPH/CYL/AXIS prescription values and a single lump Total, no line-item breakdown) that the
// prior enum (consultation/pharmacy/lab/other) forced the model to misclassify as "pharmacy" —
// it is not a drug/medicine dispensing receipt at all, so pharmacy's benefit-head signal
// (modules/ias-claim-preparation/prompts/benefit-pick.md) would have pointed
// ias-claim-preparation at the wrong BenefitType/BenefitHead for this line. Same narrow-scope
// discipline as the original voucher_type addition (20260823200000-add-voucher-type.js):
// classifies ONLY from what's visibly printed on the voucher (frame/lens/prescription specs),
// never inferred from the claim form's own "Type of Patient: optical care" field.
...
const VOUCHER_TYPE_DESCRIPTION =
  ...
  'on a requisition or result slip. "optical": eyewear order/receipt — frame/lens details, a prescription table ' +
  '(SPH/CYL/AXIS/ADD/PD), an optical shop name — not a drug dispensing receipt even if it carries a dispensing-' +
  'style stamp. "other": legible and priced but does not clearly fit one of those (e.g. genuinely mixes ' +
  'consultation and medicine on one slip). Null only when the voucher itself could not be assessed at all (e.g. ' +
  'illegible) — not a substitute for "other" when it is legible but ambiguous; those are different things.';
