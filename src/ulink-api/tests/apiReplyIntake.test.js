...
const incomplete = { id: 'case-1', currentStatus: 'API_INCOMPLETE' };
...
it('waits while there is no new reply', async () => {
  expect(await job.process({ caseRecord: incomplete, input: {} })).toMatchObject({ wait: true });
});
...
it('restarts reading when a reply brings new attachments', async () => {
  ...
  const result = await job.process({ caseRecord: incomplete, input: {} });
...
it('handles each reply once over a long thread and skips re-attached identical files', async () => {
  reply('msg-1');
  attach('att-1', 'msg-1', 'medical-report.pdf');
  const round1 = await job.process({ caseRecord: incomplete, input: {} });

  reply('msg-2');
  attach('att-2', 'msg-2', 'medical-report.pdf'); // the same file sent again
  attach('att-3', 'msg-2', 'voucher.jpg');
  const round2 = await job.process({ caseRecord: incomplete, input: { 'api-reply-intake': round1.output } });
  ...
  expect(await job.process({ caseRecord: incomplete, input: { 'api-reply-intake': round2.output } })).toMatchObject({ wait: true });
});
