...
- `invoices`: a claim can have **more than one separate voucher** — e.g. a hospital consultation receipt and a separate pharmacy receipt are two distinct physical documents, not one. Give each its own entry in `invoices.items`, each with its own `subtotal`. Do not merge them into one entry and do not assume there's only one — check every page for a second (or third) receipt before concluding there's just one. `invoices.present`: true if at least one voucher was found at all.
  ...
  - `has_clinic_stamp_or_doctor_signature`: true only if that specific voucher carries an official clinic/hospital stamp, or the treating doctor's own signature together with the doctor's personal stamp. A pre-printed, official-looking receipt from a clinic/hospital's own register (with its own letterhead, slip number, cashier ID, etc.) counts as true even without a separate ink stamp — the printed institutional format is itself the authentication. False if the voucher is a handwritten note/slip with no such mark, or only carries an unrelated mark (e.g. a pharmacy/dispensary's own stamp confirming medicine handout, or a drug product's own package/brand name printed on a dispensing slip — neither is clinic/doctor certification). Null only if the voucher couldn't be assessed at all (e.g. illegible).
