...
async function gatherAttachments(caseId) {
  ...
  for (const thread of threads) {
    for (const message of thread.EmailMessages) {
      ...
      // owning attachment's own transcribeLinkedDocuments call below, not as independent
      // top-level attachments (avoids double-counting their transcript on a reprocess).
      // NOT the same thing as sourceUrl alone: a body-linked attachment (fetched directly
      // from the email body by email-intake — see modules/email-intake/service.js — no
      // parent attachment, sourceUrl set but parentAttachmentId null) has nothing to be
