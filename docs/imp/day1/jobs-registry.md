...
| `console-upload` | `/api/jobs/console-upload/run` | `MEMBER_VERIFIED` **and** `recognizedType='ayas_member_claim'` (the AYAS reimbursement route only, not every cleared case — written explicitly so a second route added later isn't silently swept in) | `DOCUMENTS_UPLOADED` | Added 2026-09-15, sits between `document-checking`/`email-sender` and `ias-claim-preparation`. Gathers every inbound attachment on the case (`modules/shared/gatherAttachments.js`, extracted from `claim-recognition` — same query, now shared), copies each (original stays in `STORAGE_ROOT` untouched — copy, not move) into `CONSOLE_UPLOAD_ROOT` (separate root from the app's own attachment store) under `yyyy/MM/dd/{ddMMyyyy}{claimantName}-AYAS-{caseId}/` — the date is when this job actually runs, not the original email-receipt date or the OCR-extracted `claim.date_submitted`. Generates a barcode (format drafted in `docs/imp/demo/20260914/samples/console proto/demo_barcode_logic.md`) and sets `Case.consoleBarcode` + `Case.consoleUploadResult` (folder + file list). Technical failure (disk I/O error) leaves the case at `MEMBER_VERIFIED` for retry, same pattern as every other job. |
| `ias-claim-preparation` | `/api/jobs/ias-claim-preparation/run` | `DOCUMENTS_UPLOADED` (changed from `MEMBER_VERIFIED` 2026-09-15 when `console-upload` was inserted in between, so `Case.consoleBarcode` is guaranteed to exist by the time this job needs it) | `CLAIM_PAYLOAD_PREPARED` | Picks the best ICD-10 diagnosis code once per case (vector search in `modules/icd10/` + one LLM re-rank call, `modules/ias-claim-preparation/diagnosisPicker.js`), then builds **one `Items[]` line per real voucher** (`extractedFields.invoices.items[]` — verified against real sample data that a claim can genuinely be multiple separate vouchers, e.g. a consultation receipt + a separate pharmacy receipt), each with its own subtotal and its own BenefitType/BenefitHead pick against the member's own plan's valid combinations (`modules/ias-claim-preparation/benefitPicker.js`, using that voucher's own `voucher_type` — `consultation`/`pharmacy`/`lab`/`optical`/`other`, extracted by `claim-recognition` from what's visibly printed on it — as the strongest signal). Also computes STP (straight-through processing) eligibility (`modules/ias-claim-preparation/stpEligibility.js`): sums the lines' `PresentedAmt`, looks up `ulink_stp_limits` by route+currency (MMK is the only currency this system handles), and sets `Case.isStp`. Builds the full `CL_CLAIM_API` request payload (`modules/ias-claim-preparation/payloadBuilder.js`, verified against the real sample in `docs/imp/day1/IAS/ias_claim_submission_api.json`, plus `barcode`/`docCompleteDate`/`claimStatus` — none in that real sample, all provisional pending IAS confirmation: `barcode` from `Case.consoleBarcode`, `docCompleteDate` from `Case.consoleUploadResult.completedAt` (when console-upload archived the docs), `claimStatus` a fixed `"CL_STATUS_RA"` on every submission regardless of STP status — and `isValidation`/`isCSR` `"Y"`/`"Y"` when `isStp` is true, `"N"`/`"N"` otherwise). Sets `Case.iasClaimPayload`. A diagnosis/benefit pick that comes back `null` does not block preparation — that line's fields stay null; that's IAS's own validation to catch, not pre-empted here. Technical failure leaves the case at `DOCUMENTS_UPLOADED` for retry. Cases recognized before `voucher_type` existed won't have it — need reprocessing via `claim-recognition` to get accurate per-line benefit picks. |
...
| `ias-claim-stp` | `/api/jobs/ias-claim-stp/run` | `CLAIM_CREATED` **and** `isStp=true` (STP claims only — non-STP claims stay on the existing `CLAIM_APPROVAL_REVIEW` manual-JD2 path, untouched) | `CSR_SENT` | Added 2026-09-15. Calls the real IAS claim-status API (`modules/ias-claim-stp/iasClaimStatusClient.js`, same `AbortController`-timeout shape as every other external call here) with `{claimNo, fromDatetime, compCodes: ['AYAS']}` (verified against the real sample `docs/imp/day1/IAS/ias_get_claim_status_request.json`), and scans `payload.results[]` for a row with `SCMA_OID_CL_STATUS=CL_STATUS_FC` **and** non-empty `FILENAME`/`PATH` — the settlement report (CSR). If no such row exists yet, that's a normal wait state, not a failure: the case is left at `CLAIM_CREATED` for the next run, no error logged. Once found, downloads the file (`downloadFile`) and writes it to `CSR_UPLOAD_ROOT/yyyy/MM/dd/{Case.claimNo}/CSR/{filename}` (own root, separate from `STORAGE_ROOT`/`CONSOLE_UPLOAD_ROOT`), queues a customer-facing `CSR_REPORT` `EmailTask` with the PDF attached (first task type in this codebase to carry an attachment — see `channels/imapSmtpChannel.js`'s `sendReply`/`modules/email-sender/templates.js`'s `renderCsrReport`), and moves the case to `CSR_SENT`. A genuine technical failure (IAS timeout/error, disk I/O error) leaves the case at `CLAIM_CREATED` for retry, same pattern as every other job — kept distinct from the normal "not ready yet" path. |
...
- `POST /api/dev/console-upload/:caseId/preview` — zero persistence, no disk writes, no external call.
...
- `POST /api/dev/ias-claim-stp/:caseId/preview` — real IAS `getClaimStatus` call, zero persistence, never calls `downloadFile` or writes to disk.
...
POST /api/jobs/pipeline/run   ← the only crontab line
  ...
  → console-upload.run()          ← added 2026-09-15; AYAS reimbursement route only; MEMBER_VERIFIED → DOCUMENTS_UPLOADED
  → ias-claim-preparation.run()    ← reads DOCUMENTS_UPLOADED, not MEMBER_VERIFIED, as of 2026-09-15
  → ias-claim-creation.run()
  → ias-claim-stp.run()            ← added 2026-09-15; isStp claims only; CLAIM_CREATED → CSR_SENT
  → email-sender.run()            ← catches ias-claim-creation's CLAIM_CREATED_NOTIFICATION and ias-claim-stp's CSR_REPORT, same run
```
