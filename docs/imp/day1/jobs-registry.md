...
POST /api/jobs/pipeline/run   ← the only crontab line
  ...
  → email-sender.run()            ← catches document-checking's and member-verification's tasks
...
three different producers (`document-checking`, `member-verification`, and
